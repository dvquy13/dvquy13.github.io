...
This chapter focuses on the data pipeline from raw data to model-ready features. We'll explore the Amazon Reviews dataset, understand user-item interaction sequences, and build the feature engineering pipeline that will power our session-based recommender.
...
As you can see above, the train dataset has 8M ratings for us to play with. That's not a small number. But if you're getting excited about the perspective of using big data framework to transform those interactions, I'm sorry to let you down. The first thing I normally do with raw data is **not** to utilize all the data points. In fact, I even go the opposite direction. The next section provides some insights into why we may consider sampling data as the first thing to do for our project.

## Data Sampling and Filtering

If you study the documentation of our Amazon Reviews dataset, you will find that there are two subsets of the data:
...
In the end, we won't have exactly 10,000 users, but the numbers will be close. After sampling, here is the distribution of interactions:
