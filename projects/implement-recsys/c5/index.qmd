...
image: "../static/sequence-modeling.png"
format:
  html:
    code-fold: false
...
This happens because our model doesn't need to know who you are. It mostly needs to know what you're doing. Traditional collaborative filtering methods like Matrix Factorization, while also leaning on user interaction data, normally provides user embedding that captures stable preferences over time. The sequence representation captures what's happening right now. This is largely debateable, but in a lot of the settings "right now" is often more important than "over time" for making the next recommendation[^1].

[^1]: The beauty of neural network-based approaches is that they offer flexibility to design an architecture that combines and best captures from multiple sources of signals.
...
```
1. In case we are using a pre-trained item embedding, we can pass it in as an argument to the model.
...
After training, we can compare the performance of our model with the popularity baseline on the same set of evaluation metrics:

:::{.column-page}
![](../static/model-compare.png)
:::

As you can see, our personalized model is able to outperform the popularity baseline on all aspects by quite a large margin.
