...
# ///
"""
Fetch total reaction count across all Giscus discussions.

Giscus stores comments and reactions as GitHub Discussions in the configured repo.
This script queries the GitHub GraphQL API, paginates through all discussions,
and sums every reaction type (👍 ❤️ 😄 🎉 😕 👀 🚀) across all posts.

Auth: GITHUB_TOKEN env var (required — GitHub GraphQL API requires auth even for public repos)

Usage:
  uv run scripts/fetch-giscus-reactions.py
  uv run scripts/fetch-giscus-reactions.py --test    # exit 1 if error
  uv run scripts/fetch-giscus-reactions.py --fixture # offline mode (reads fixtures/giscus_total_reactions.txt)

Output (same format as fetch-metrics.py):
  {"fetched_at": "...", "metrics": {"giscus_total_reactions": {"value": 42, "status": "ok", "fetched_at": "..."}}}
"""
...
METRIC_NAME = "giscus_total_reactions"
...
def fetch_total_reactions(token: str) -> int:
    total = 0
    cursor = None
...
def main():
    parser = argparse.ArgumentParser(description="Fetch total Giscus reactions via GitHub GraphQL API")
    ...
    try:
        if args.fixture:
            ...
            value = fetch_total_reactions(token)
