...
- **Short punchy fragments for dramatic effect** — `It worked.` / `Not archived. Deleted from disk.` / `So I built one.` / `Move the threshold and every number changes.` (a command clause followed by a flat, disconnected consequence clause is the same cadence, just dropped mid-paragraph instead of used standalone). These read as AI slop. Integrate them into the surrounding sentence.
